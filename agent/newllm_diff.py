...
def build_structured_function_results(
    ...
    for index, raw_result in sorted(gathered_results, key=lambda item: item[0]):
        ...
        if isinstance(raw_result, str) and isinstance(parsed_result, dict):
            try:
                m = re.search(r"初次分析结果:\s*```json\s*(\{[\s\S]*?\})\s*```", raw_result)
                if m:
...
def _build_direct_compact_topk_prompt(
    *,
    ...
    return f"""根据下方 CVE Description，在 Candidates 中找出根因函数并按与 CVE 描述信息的相关性从高到低排序返回前 {expected} 名，输出仅 JSON：{{"primary_cve_root_function": <string or null>, "ranked_candidates": [{{"function_name": <string>, "relevance_score": <int>, "rank": <int starting from 1>}}, ...]}}。
...
"""
    ...
    async def async_query2bot(self, fa, fb, cve_details=None, cwe=None, work_mode: str = "reproduction") -> str:
        ...
        # 检查是否需要进行二次判断（ReAct / RAG 第二阶段）
        # 注意：本逻辑为消融实验服务，可通过 self.react_trigger 控制触发条件：
        # - never: 不进入第二阶段
        # - scenario_yes: scenario_match==Yes 即进入
        # - scenario_and_property_yes: scenario_match==Yes 且 property_match==Yes 才进入
...
async def main(chat_id: str,
    ...
    # 策略 1：不做任何过滤/二阶段分析，直接对所有变更函数做 root-cause Top-20 排序
    if strategy == 1:
        direct_candidates = [
        ...
        try:
            ...
            logger.error(f"写入 strategy{strategy} Top-20 结果失败: {e}")
