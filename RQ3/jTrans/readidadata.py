...
def _warn_empty_operand(code, operator, which):
    sys.stderr.write(f"Empty operand{which} in asm for operator '{operator}': {code!r}\n")
...
def parse_asm(code):   #handle ida code to better quality code for NLP model
    ...
    # Strip ';' comments only (keep '#' content for backward-compatible tokens)
    if code.find(';')!=-1:
        id=code.find(';')
    ...
    if operand!=None:
        if operand.find(',')!=-1:
            ...
            operand1=operand
            operand2=None
    if operand1 is not None and operand1.strip(' ') == '':
        _warn_empty_operand(code, operator, 1)
        operand1 = None
    if operand2 is not None and operand2.strip(' ') == '':
        _warn_empty_operand(code, operator, 2)
        operand2 = None
    if operand3 is not None and operand3.strip(' ') == '':
        _warn_empty_operand(code, operator, 3)
        operand3 = None
    if operand1!=None:
