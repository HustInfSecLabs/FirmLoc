...
To support artifact evaluation, we provide a representative subset that is sufficient to verify the methodology and core claims of the paper. The subset includes one CVE per vendor, deliberately selected to cover the three primary vulnerability classes evaluated in our study: injection vulnerabilities, buffer overflow vulnerabilities, and path traversal vulnerabilities. This selection ensures that the subset reflects the diversity of both vendor ecosystems and vulnerability types present in the full dataset, and that all major components of the FirmLoc pipeline — CVE context retrieval, binary diffing, dataflow analysis, and function ranking — can be exercised end-to-end on the provided samples.
